...
void simulate(double lambda, double mu) {
	...
	while (arrivals < NUMBER_OF_PACKETS) {
		if (exponentialGenerator(lambda, timestep)) { 
			arrivals++; // increment the nubmer of arrivals
/* min queue assignment*/if (queue1Length <= queue2Length) { //assigned to queue 1
...
int main(int args, char** argv) {
	...
	printf("\n--------------------------------------------------------------------\n");
	printf("Results are calculated over %d iterations:\n", ITERATIONS);
